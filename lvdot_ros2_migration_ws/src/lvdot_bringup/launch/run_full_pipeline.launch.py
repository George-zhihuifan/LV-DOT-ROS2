"""Full pipeline launch: Gazebo scene + realistic_sensors + YOLO + detector + QC fusion + GRU + RViz.

Single-command bring-up of the production main-line pipeline:

  scene (pedestrian_prototype.sdf, 8 actors, UavPoseSyncSystem plugin)
   ├─ rgbd_camera, gpu_lidar (raw Gazebo)
   ├─ d435i_sim, mid360_sim (realistic noise overlay)
   ├─ pedestrian_state_publisher (GT)
   └─ ros_gz_bridge

  pose_stub (orbit, 25Hz) → /mavros/local_position/{pose,odom} + /uav_motion/pose_cmd
   ↓
  lvdot_yolo_node (Ultralytics yolo11n, GPU1, FP16) → /yolo_detector/detected_bounding_boxes
   ↓
  lvdot_detector_main (depth U-V + lidar DBSCAN + Kalman tracking)
   ├─ /onboard_detector/visual_bboxes_qcgaf
   ├─ /onboard_detector/lidar_bboxes_qcgaf
   └─ /onboard_detector/dynamic_bboxes
   ↓
  qcgaf_fusion_node (QC-GAF, GPU1)  → /qcgaf/fused_bboxes
   ↓
  gru_prediction_node (Kalman+GRU hybrid, GPU0)  → /gru_predictor/predicted_positions

  RViz (depending on rviz arg)

Usage:
  ros2 launch lvdot_bringup run_full_pipeline.launch.py \
    gazebo_gui:=true rviz:=true \
    qcgaf_checkpoint:=<ws_root>/models/qcgaf/best_model.pt \
    gru_model:=<ws_root>/models/gru/best_model.pth
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    default_detector_config = str(bringup_share / "config" / "detector_param.yaml")
    depth_eval_share = Path(get_package_share_directory("depth_eval_bringup"))
    default_scenario_config = str(depth_eval_share / "config" / "pedestrian_prototype.yaml")
    ...
    return LaunchDescription([
        ...
        # Scene + sensors + detector + YOLO + pose_stub (launch_evaluator wired in scene launch)
        ...
        # QC-GAF fusion node — pinned to GPU1 via additional_env
        Node(
        ...
        # GRU predictor — pinned to GPU0 to avoid contending with YOLO/QC-GAF on GPU1.
        Node(
        ...
        # RViz with detector preset (shows depth, lidar, all box layers)
        Node(
            package="rviz2",
            executable="rviz2",
