...
# One-click full fusion stack:
# depth + UV/DB + LiDAR + YOLO + fusion + RViz

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RVIZ_CONFIG="${SCRIPT_DIR%/scripts}/rviz/lvdot_detector.rviz"
LAUNCH_LOG="/tmp/start_full_fusion_stack.launch.log"
RVIZ_LOG="/tmp/start_full_fusion_stack.rviz.log"
...
stop_all() {
  cleanup_stale
  rm -f /tmp/start_full_fusion_stack.launch.pid /tmp/start_full_fusion_stack.rviz.pid
...
start_chain() {
  nohup ros2 launch lvdot_bringup run_detector_with_scene.launch.py \
    ...
    executor_threads:=4 \
    > "${LAUNCH_LOG}" 2>&1 &
  echo $! > /tmp/start_full_fusion_stack.launch.pid
}

start_rviz() {
  nohup rviz2 -d "${RVIZ_CONFIG}" > "${RVIZ_LOG}" 2>&1 &
  echo $! > /tmp/start_full_fusion_stack.rviz.pid
}

print_status() {
  echo "[start] launch_log=${LAUNCH_LOG}"
  echo "[start] rviz_log=${RVIZ_LOG}"
  if timeout 8 ros2 topic echo /onboard_detector/pipeline_stats_status --once >/tmp/start_full_fusion_stack.pipeline.txt 2>/dev/null; then
    echo "[start] pipeline snapshot:"
    sed -n '1,120p' /tmp/start_full_fusion_stack.pipeline.txt
  else
