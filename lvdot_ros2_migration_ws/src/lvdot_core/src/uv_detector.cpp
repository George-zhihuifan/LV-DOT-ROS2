 ...
 * -------------------------------------------------------------------------
 * Ported verbatim from the ROS1 LV-DOT onboard_detector/uvDetector.cpp.
...
namespace onboardDetector{
    ...
    void UVdetector::extract_U_map()
    {
        ...
        for(int col = 0; col < depth_rescale.cols; col++)
        {
            for(int row = 0; row < depth_rescale.rows; row++)
            {
                ...
                depth_rescale_val = static_cast<int>(depth_rescale.at<unsigned short>(row, col));
    ...
    {   
        ...
        int valid_depth_rows = 0;
        int invalid_depth_rows = 0;
        for (size_t b = 0; b < this->bounding_box_U.size(); b++) {
            ...
            const int col_start = std::max(0, x);
            const int col_end = std::min(depth_resize.cols, x + width);
            // j + check + 1 must stay within [0, rows - 1]
            const int max_j = std::max(0, depth_resize.rows - num_check - 1);
            for (int i = col_start; i < col_end; i++) { // for several middle columns
                for (int j = 0; j < max_j; j++) { // for each row (bounded to avoid OOB)
                    depth_resize_val = static_cast<float>(depth_resize.at<unsigned short>(j, i));
                    if (float(depth_resize_val) >= depth_in_near && depth_resize_val <= depth_in_far) {
                        for (int check = 0; check < num_check; check++) { // check some more points in the coloum
                            depth_resize_val = static_cast<float>(depth_resize.at<unsigned short>(j + check + 1, i));
        ...
        if (!this->bounding_box_U.empty()) {
            printf(
                "[UV-3D] u_boxes=%zu d_boxes=%zu valid_rows=%d invalid_rows=%d\n",
                this->bounding_box_U.size(),
                this->box3Ds.size(),
                valid_depth_rows,
                invalid_depth_rows);
        }
