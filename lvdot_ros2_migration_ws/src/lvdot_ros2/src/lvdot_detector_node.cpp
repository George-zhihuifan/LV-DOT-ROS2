...
: rclcpp::Node("lvdot_detector_node", options)
{
  ...
  if (config_.enable_sync_context) {
...
void LVdotDetectorNode::load_config_from_parameters()
{
  ...
  config_.skew_startup_grace_sec = resolve_parameter<double>(
    *this, "skew_startup_grace_sec", config_.skew_startup_grace_sec);
  config_.future_stamp_tolerance_sec = resolve_parameter<double>(
    *this, "future_stamp_tolerance_sec", config_.future_stamp_tolerance_sec);
  ...
  if (config_.time_step <= 0.0) {
    RCLCPP_WARN(get_logger(), "Invalid time_step=%.3f, fallback to 0.033", config_.time_step);
    config_.time_step = 0.033;
  }
  if (config_.lidar_dbscan_epsilon <= 0.0) {
    RCLCPP_WARN(
      get_logger(), "Invalid lidar_DBSCAN_epsilon=%.3f, fallback to 0.05",
      config_.lidar_dbscan_epsilon);
    config_.lidar_dbscan_epsilon = 0.05;
  }
  if (config_.lidar_dbscan_min_points <= 0) {
    RCLCPP_WARN(
      get_logger(), "Invalid lidar_DBSCAN_min_points=%d, fallback to 10",
      config_.lidar_dbscan_min_points);
    config_.lidar_dbscan_min_points = 10;
  }
  if (config_.max_depth_lidar_skew_sec <= 0.0) {
    config_.max_depth_lidar_skew_sec = 0.8;
  }
  if (config_.max_depth_yolo_skew_sec <= 0.0) {
    config_.max_depth_yolo_skew_sec = 0.8;
  }
  ...
  if (config_.skew_startup_grace_sec < 0.0) {
    config_.skew_startup_grace_sec = 0.0;
  }
...
void LVdotDetectorNode::create_publishers()
{
  ...
  visual_bboxes_qcgaf_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
    "onboard_detector/visual_bboxes_qcgaf", marker_qos);
  lidar_bboxes_qcgaf_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>(
    "onboard_detector/lidar_bboxes_qcgaf", marker_qos);
...
void LVdotDetectorNode::on_detection_timer()
{
  ...
  const double uptime_sec = std::max(0.0, (tick_now - node_start_time_).seconds());
  const bool in_startup_grace = uptime_sec < config_.skew_startup_grace_sec;

  auto message_age_sec = [&](const rclcpp::Time & stamp) -> double {
      return (tick_now - stamp).seconds();
    };
  auto message_is_stale_or_future = [&](const rclcpp::Time & stamp) -> bool {
      const double age = message_age_sec(stamp);
      return age > config_.stale_message_age_sec || age < -config_.future_stamp_tolerance_sec;
    };
...
void LVdotDetectorNode::on_vis_timer()
{
  ...
  lidar_bboxes_qcgaf_pub_->publish(
    make_box_markers(
      snapshot.lidar_bboxes, frame_id, "lidar_qcgaf_input",
      make_color(0.5f, 0.5f, 0.5f, 0.35f), stamp));
