...
def generate_launch_description() -> LaunchDescription:
    ...
    lvdot_bringup_share = get_package_share_directory('lvdot_bringup')
    plugin_prefix = get_package_prefix('depth_eval_ped_gz_plugin')
    world_path = os.path.join(pkg_bringup, 'worlds', 'pedestrian_clean_6agents_staticobs.sdf')
    model_path = os.path.join(pkg_bringup, 'models')
    default_config_path = os.path.join(
        lvdot_bringup_share, 'config', 'clean_scenarios', 'pedestrian_clean_06agents_staticobs.yaml')
    config_path = LaunchConfiguration('scenario_config')
    rviz_config = os.path.join(pkg_bringup, 'rviz', 'uav_pedestrian_scene.rviz')
    world_name = 'pedestrian_clean_6agents_staticobs'
